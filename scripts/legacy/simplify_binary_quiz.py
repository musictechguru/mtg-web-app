#!/usr/bin/env python3
"""
Script to simplify Binary MIDI quiz - remove HTML from Q1
"""
...
if topic_30_index is not None:
    ...
    print(f"Updated Topic 30 - simplified Q1 (removed HTML content)")
else:
    stage3['items'].append(binary_quiz)
    print(f"Added new Topic 30")

print(f"Stage 3 now has {len(stage3['items'])} items")
print(f"Quiz now has {len(binary_quiz['questions'])} clean binary-diagram questions")

# Write back to file
with open('src/data/course_data.json', 'w') as f:
    json.dump(data, f, indent=4)

print("Success! Simplified Binary MIDI quiz updated")
