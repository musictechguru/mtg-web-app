...
class SVG:
    ...
    def add_text(self, x, y, text, font_size=20, fill="black", text_anchor="start", font_weight="normal"):
        self.elements.append(f'<text x="{x}" y="{y}" font-family="Arial, sans-serif" font-size="{font_size}" fill="{fill}" text-anchor="{text_anchor}" font-weight="{font_weight}">{text}</text>')
...
def draw_graph_frame(svg, title, x_label, y_label):
    ...
    svg.add_line(ox, oy, ox+w, oy, stroke="#333", stroke_width=2) # X
    svg.add_line(ox, oy, ox, oy-h, stroke="#333", stroke_width=2) # Y
...
def gen_rt60(path):
    ...
    # Decay curve points (exponential decay)
    points = []
    for i in range(101):
        x = i / 100 # 0 to 1
        y = math.exp(-3 * x) # Decay
        px = ox + x * w
        py = oy - y * h
        points.append((px, py))
    ...
    s.add_line(ox, oy - h*0.01, ox+w, oy-h*0.01, stroke="#ef4444", stroke_width=2, stroke_dasharray="5,5") # -60dB line (conceptually)
...
def gen_compression(path):
    ...
    # Compressed after threshold (4:1 slope)
    end_x = ox + w
    end_y = threshold_y - (h * 0.5 * 0.25) # 1/4 slope
...
def gen_eq_bell(path):
    ...
    center_x = ox + w/2
    peak_y = oy - h * 0.8
    base_y = oy - h/2 # 0dB line
    
    s.add_line(ox, base_y, ox+w, base_y, stroke="#ccc", stroke_width=1) # 0dB
...
def main():
    ...
    # This is a sample mapping. In a full implementation, I'd map all 139.
    # For now, I map broadly by string matching to a few types.
    ...
    print(f"Processed {len(missing)} images.")
