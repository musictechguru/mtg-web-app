...
def audit_vol2():
    ...
    report_lines = []
    report_lines.append("# Audit Report: Volume 2 Content")
        ...
        for topic in part['topics']:
            ...
            for i, q in enumerate(topic['levels']['basic']):
                ...
                if not image_data:
                    ...
                    if src.startswith('/'):
                        # Assuming src relative to public
                ...
                if failures:
                    topic_issues.append(f"- **Q{i+1}** ({q_text}): {', '.join(failures)}")
                    issues_found += 1
    ...
    output_path = os.path.join(base_path, 'src/data/audit_report_vol2.md')
