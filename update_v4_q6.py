...
img_path = '/images/Dictiionary_Quiz_image_Pool/percussive_fade_hq.png'
    ...
    for part in vol4['parts']:
        for topic in part['topics']:
            for level_key, questions in topic['levels'].items():
                for q in questions:
                    if "percussive sounds like drums" in q.get('content', '').lower() or "how long should a fade be" in q.get('content', '').lower():
                        if 'img' in q:
                            q['img'] = img_path
                        else:
                            q['img'] = img_path
