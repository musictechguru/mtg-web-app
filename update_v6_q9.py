...
img_path = '/images/Dictiionary_Quiz_image_Pool/eq_lowcut_hq.png'
    ...
    for part in vol6['parts']:
        for topic in part['topics']:
            for level_key, questions in topic['levels'].items():
                for q in questions:
                    if "remove the 20-60hz range from most instruments except kick and bass" in q.get('content', '').lower() or "remove the 20-60hz range" in q.get('content', '').lower():
                        if 'img' in q:
                            q['img'] = img_path
                        else:
                            q['img'] = img_path
