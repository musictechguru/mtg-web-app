...
img_path = '/images/Dictiionary_Quiz_image_Pool/eq_presence_hq.png'
    ...
    for part in vol6['parts']:
        for topic in part['topics']:
            for level_key, questions in topic['levels'].items():
                for q in questions:
                    if "\"presence\" range where clarity and intelligibility live" in q.get('content', '').lower() or "often called the \"presence\" range" in q.get('content', '').lower():
                        if 'img' in q:
                            q['img'] = img_path
                        else:
                            q['img'] = img_path
