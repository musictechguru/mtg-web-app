...
img_path = '/images/Dictiionary_Quiz_image_Pool/eq_meaning_hq.png'
    ...
    for part in vol6['parts']:
        for topic in part['topics']:
            for level_key, questions in topic['levels'].items():
                for q in questions:
                    if "what does eq stand for" in q.get('content', '').lower() or "stand for in music production" in q.get('content', '').lower():
                        if 'img' in q:
                            q['img'] = img_path
                        else:
                            q['img'] = img_path
